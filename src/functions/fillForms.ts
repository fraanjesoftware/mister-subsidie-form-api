...
interface Results {
  filled: FillResult[];
  errors: ErrorResult[];
  driveUpload: DriveUploadResult | null;
  pdfData?: Array<{
    form: string;
    filename: string;
    base64: string;
    ...
    try {
      ...
      if (requestBody.deMinimis) {
        try {
          ...
          if (typeof result === 'string') {
            throw new Error('Unexpected string result from fillDeMinimisForm');
          }
      ...
      if (requestBody.machtiging) {
        try {
          ...
          if (typeof result === 'string') {
            throw new Error('Unexpected string result from fillMachtigingsformulier');
          }
      ...
      if (requestBody.uploadToDrive !== false && pdfFiles.length > 0) {
        ...
        results.pdfData = pdfFiles.map(file => ({
          ...
          base64: Buffer.from(file.pdfBytes).toString('base64')
        }));
      }
      ...
      if (process.env.NODE_ENV === 'development' || results.errors.length > 0) {
        (response as any).debug = results.debug;
      }
